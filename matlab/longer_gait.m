...
% FootHeight = 0 ;
figure
t_vec_cycle = linspace(0,T_cycle,401);
zr = [A*sin(pi*linspace(0,1,101)).^2, zeros(1,300)];
xr = [(0.8*linspace(0, 0.5, 51).^2)/2, -(0.8*linspace(0.01, 0.5, 50).^2)/2 + 0.4*linspace(0.01, 0.5, 50) + 0.1, 0.2*ones(1,300)];


zl = [zeros(1,200), A*sin(pi*linspace(0,1,101)).^2, zeros(1,100)];
xl = [0.1*ones(1,200), (0.8*linspace(0, 0.5, 51).^2)/2 + 0.1, -(0.8*linspace(0.01, 0.5, 50).^2)/2 + 0.4*linspace(0.01, 0.5, 50) + 0.2, 0.1 + 0.2*ones(1,100)];

plot(t_vec_cycle,xr,'LineWidth',2)
hold on
plot(t_vec_cycle,zr,'LineWidth',2)
plot(t_vec_cycle,xl,'LineWidth',2)
plot(t_vec_cycle,zl,'LineWidth',2)
...
% data_to_export = [t_vec_cycle; xr; zr; xl; zl];


% csvwrite('longergait.csv',data_to_export)
...
% 
% figure
% plot(t_vec_new,xr_new)
% hold on
% plot(t_vec_new,zr_new)
% plot(t_vec_new,xl_new)
% plot(t_vec_new,zl_new)
% legend('xr','zr','xl','zl')
% 
